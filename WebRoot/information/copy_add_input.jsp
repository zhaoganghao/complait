						...
						<tbody>
								...
								<td class="selectArea"><input type="radio"
									...
									<select id="aid" name="aid" onchange="getCities();">
										...
										<c:forEach items="${citys}" var="city">
											<c:if test="${copy.aid.aid==city.aid }">
								...
								<td class="selectArea"><input type="radio"
									...
									name="collegeaid">
										...
										<c:forEach items="${colleges}" var="college">
											<c:if test="${copy.aid.aid==college.aid }">
							...
							<tr>
								...
								<td><select id="sid" name="sid" id="label">
										<c:forEach items="${sourceList}" var="source">
											<c:if test="${copy.sid.sid==source.sid }">
												<option value="${source.sid}" selected="selected">${source.name}</option>
											</c:if>
											<c:if test="${copy.sid.sid!=source.sid }">
							...
							<tr>
								...
								<td><select id="did" name="did" id="label2">
										<c:forEach items="${diplomaList}" var="diploma">
											...
											<c:if test="${copy.did.did==diploma.did }">
												<option value="${diploma.did}" selected="selected">${diploma.name}</option>
											</c:if>
											<c:if test="${copy.did.did!=diploma.did}">
							...
							<tr>
								...
								<td><select id="cid" name="cid" id="label3">
										<c:forEach items="${ccList}" var="cc">
											<c:if test="${copy.cid.cid==cc.cid }">
												<option value="${cc.cid}" selected="selected">${cc.name}</option>
											</c:if>
											<c:if test="${copy.cid.cid!=cc.cid }">
							...
							<tr>
								...
								<td><input id="ifrom" name="ifrom" type="text"
									id="textfield" value="${copy.ifrom }"
									onblur="strValidate('ifrom',10)" /><span>不能为空，字符不能超过10个</span>
								</td>
							</tr>
							<tr>
								<td>编号
								...
								<td><input id="encode" name="encode" type="text"
									value="${copy.encode }" id="textfield"
									onblur="strValidate('encode',5)" /><span>不能为空，字符不能超过5个</span>
								</td>
							</tr>
							<tr>
								<td>被举报单位(或学校)
							...
							<tr>
								...
								<td><textarea id="content1" name="content"
										onblur="nullValidate('content1',10)">${copy.content}</textarea><span>不能为空</span>
								</td>
							</tr>
							<tr>
								<td>处理意见和建议
								...
								<td><textarea id="content2" name="remarks"
										onblur="nullValidate('content2',10)">${copy.remarks }</textarea><span>不能为空</span>
								</td>
							</tr>
							<tr>
								<td>所限回复日期
								...
								<td><input type="text" name="limit_time"
									id="l_picker_start" value="${copy.limit_time }"
									onblur="dateValidate('l_picker_start',10)" /><span>格式为：2013-01-01</span>
								</td>
							</tr>
							<tr>
								<td>经办人
								...
								<td><input id="operater" name="operater" type="text"
									value="${copy.operater }" onblur="strValidate('operater',5)" /><span>不能为空，字符不能超过5个</span>
								</td>
							</tr>
							<tr>
								<td>回复电话
							...
							<tr>
								...
								<td><input id="replyemail" name="replyemail" type="text"
									value="${copy.replyemail }"
									onblur="emailValidate('replyemail',20)" /><span>不能为空，字符不能超过20个,格式要正确</span>
								</td>
							</tr>
							<tr>
								<td colspan="2"><input type="button" value="保存修改"
									onclick=" submit1()">&nbsp;<input type="button" value="提交投诉" class="btn btn-primary"
									onclick="submit2()" class="btn btn-primary">
								</td>
							</tr>
						</tbody>
					</table>
