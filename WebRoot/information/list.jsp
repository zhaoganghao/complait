...
<link rel="stylesheet" type="text/css"${page.pagenum}
	href="${pageContext.request.contextPath}/stylesheets/paging.css">
<div id="paging">
	<span>当前第[${page.pagenum }]页</span> <span><c:if
			test="${page.pagenum>1}">
			<a href="${page.url }?pagenum=${page.pagenum-1 }&method=${methodname}&aid=${param.aid}&caid=${param.caid}&collegeaid=${param.collegeaid}
			&datestart=${param.datestart}&dateend=${param.dateend}&cid=${param.cid}&sid=${param.sid}&did=${param.did}">上一页</a>
		</c:if> 
		</span> <span> <c:forEach var="pagenum" begin="${page.startPage}"
			end="${page.endPage}">
		[<a href="${page.url }?pagenum=${page.pagenum-1 }&method=${methodname}&aid=${param.aid}&caid=${param.caid}&collegeaid=${param.collegeaid}
			&datestart=${param.datestart}&dateend=${param.dateend}&cid=${param.cid}&sid=${param.sid}&did=${param.did}">${pagenum }</a>]
	</c:forEach>
	 </span> <span> <c:if test="${page.pagenum<page.totalpage}">
		<a href="${page.url }?pagenum=${page.pagenum-1 }&method=${methodname}&aid=${param.aid}&caid=${param.caid}&collegeaid=${param.collegeaid}
			&datestart=${param.datestart}&dateend=${param.dateend}&cid=${param.cid}&sid=${param.sid}&did=${param.did}">下一页</a>
